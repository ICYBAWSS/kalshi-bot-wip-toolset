...
script_dir = '/Users/ray/kalshi-bot-env/time_graphed_data'
twitter_script = os.path.join(script_dir, 'twitter_huge.py')
tiktok_script = os.path.join(script_dir, 'tiktok_huge.py')
sentiment_script = os.path.join(script_dir, 'sentiment_analysis_huge.py')
graph_script = os.path.join(script_dir, 'hugeGRAPHED.py')
...
# Run hugeGRAPHED.py with full path
print("\n--- Running hugeGRAPHED.py ---")
try:
    subprocess.run(['python3', graph_script], check=True)
except subprocess.CalledProcessError as e:
    print(f"Error running sentiment analysis script: {e}")

print("\n--- Master script execution completed ---")